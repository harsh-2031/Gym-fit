...
const Footer = () => {
  // --- NEW: Check login state ---
  const isUserLoggedIn = !!localStorage.getItem("token");
  const isTrainerLoggedIn = !!localStorage.getItem("trainerToken");

  return (
    <footer className="bg-bg-paper border-t border-gray-700 mt-auto text-text-secondary">
      <div className="container mx-auto px-4 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
          <div className="space-y-4">
            <h3 className="text-xl font-bold text-primary">Gym-Fit</h3>
            <p className="text-sm">
              Your ultimate companion in the journey to strength and wellness.
            </p>
          </div>

          {/* --- NEW: Dynamic Quick Links Section --- */}
          <div>
